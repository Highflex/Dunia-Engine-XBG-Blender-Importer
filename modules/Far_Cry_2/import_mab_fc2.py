"""MAB animation importer — clean implementation based on buu342's validated
...
def build_single_bone_timeline(d, sections):
    ...
    set bit in the group's bitmask places an extra quat at that subframe.
    """
    ...
    for g in groups:
        ...
        # bit b -> subframe b+1 (base quat covers subframe 0); the old
        # `subframe = b` reading put every extra key one frame early
        # (constant-speed clips jittered) -- see import_mab_fc3.py.
...
def apply_single_bone(context, d, sections, arm_obj, bone_name,
    ...
    if mult > 1 and len(raw) >= 2:
        from .quat_resample_fc2 import resample_rotation
...
def read_lks_bone_names(skeleton_path):
    ...
    if not (0 < root_name_len < 64):
        return []
    names = [data[78:78 + root_name_len].decode('latin-1')]
    pos = 78 + root_name_len + 1            # +0x01 terminator after root

    def name_at_k(p, k):
        """Validate (u32 len)(printable name) at block offset k."""
        if p + k + 4 > len(data):
            return None
        nl = struct.unpack_from('<I', data, p + k)[0]
        na = p + k + 4
        if not (0 < nl < 64) or na + nl + 1 > len(data):
            return None
        raw = data[na:na + nl]
        if not all(0x20 <= c < 0x7F for c in raw):
            return None
        if data[na + nl] not in (0, 1):     # block terminator
            return None
        return raw.decode('latin-1'), na + nl + 1

    for _ in range(1, bone_count):
        # Block size varies by bone type: 63 B standard, 71 B extended
        # (twist), 75 B constraint/IK (e.g. FC2 'R Knee').  The name length
        # sits 4 bytes before the name in all variants — probe candidates.
        hit = None
...
def _fcurve_container(obj, action):
    ...
    from .procedural_fc2 import fcurve_container
...
def apply_multi_bone(context, d, sections, arm_obj, xbg_path,
    ...
    n_helpers = 0
    if emulate_helpers:
        from .procedural_fc2 import emulate_procedural_helpers
